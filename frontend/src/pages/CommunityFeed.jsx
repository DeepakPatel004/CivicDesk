...
const CommunityFeed = () => {
  ...
  // NOTE: In a real application, you would get the user's ID from an authentication context or token.
  // For this example, we'll use a placeholder.
  const [currentUserId, setCurrentUserId] = useState('placeholder_user_id');

  useEffect(() => {
    // Only fetch data if a district has been selected
    if (!selectedDistrict) {
      setReports([]);
      return;
    }
    ...
    const fetchDistrictReports = async () => {
      ...
      try {
          const response = await fetch(`${API_BASE_URL}/reports/district-feed?district=${selectedDistrict}`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          // Assuming user ID is available from auth state after successful login
  ...
  // 🚀 New function to handle upvoting
  const handleUpvote = async (reportId) => {
    const token = sessionStorage.getItem('userToken');
    const API_BASE_URL = 'http://localhost:5000/api';

    if (!token) {
      alert('You must be logged in to upvote a report.');
      navigate('/signin');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        // Update the state with the new report data from the API response
        setReports(reports.map(report =>
          report._id === reportId ? { ...report, upvotes: result.report.upvotes } : report
        ));
        // Optional: show a small toast or message
        console.log(result.message);
      } else {
        alert(result.message || 'Failed to toggle upvote.');
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <select
            ...
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 bg-white shadow-sm"
          >
        ...
        {isLoading && <div className="text-center text-gray-600">Loading reports...</div>}
        ...
        {!selectedDistrict && !isLoading && (
          <div className="text-center text-gray-500 mt-12">
            ...
            <p className="mt-4 text-lg">Please select a district to view local reports.</p>
          </div>
        )}

        {selectedDistrict && !isLoading && !error && reports.length === 0 && (
          <div className="text-center text-gray-500">No reports found for {selectedDistrict}.</div>
        )}
        ...
        {selectedDistrict && !isLoading && !error && reports.length > 0 && (
          <div className="space-y-6">
            ...
            {reports.map(report => {
              const hasUpvoted = report.upvotes.includes(currentUserId);
              return (
                <div key={report._id} className="bg-white p-6 rounded-lg shadow-md">
                  ...
                  <p className="text-gray-600 mt-2">
                    <span className="font-medium">Locality:</span> {report.location.locality}
                  </p>
                  <p className="text-sm text-gray-500 mt-1">
                    Reported by: {report.submittedBy ? report.submittedBy.name : 'Unknown User'} on {new Date(report.createdAt).toLocaleDateString()}
                  </p>
                  {report.photoUrl && (
                    <img src={report.photoUrl} alt="Reported issue" className="mt-4 rounded-lg w-full max-h-80 object-cover" />
                  )}
                  ...
                  <div className="mt-4 flex items-center space-x-2">
                    ...
                    >
                      {/* Thumbs up icon */}
                      <svg
                        className={`w-5 h-5 ${hasUpvoted ? 'text-blue-500' : 'text-gray-400'}`}
                        fill={hasUpvoted ? 'currentColor' : 'none'}
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10H3.32a.75.75 0 00-.73.682l-.46 3.655a1.5 1.5 0 001.498 1.663h14.735a1.5 1.5 0 001.498-1.663l-.46-3.655a.75.75 0 00-.73-.682H14z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10V5a2 2 0 00-2-2h-2a2 2 0 00-2 2v5m4 0V5m4 5v5a2 2 0 01-2 2h-2a2 2 0 01-2-2v-5" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 21v-4" />
                      </svg>
                      <span className={`text-sm font-medium ${hasUpvoted ? 'text-blue-600' : 'text-gray-600'}`}>
...
//   // Fetch all public reports from the user's district
//   useEffect(() => {
//     const API_BASE_URL = 'http://localhost:5000/api';
//     const fetchDistrictReports = async () => {
//         console.log(12)
...
//       try {
//         console.log(12)
//         const response = await fetch(`${API_BASE_URL}/reports/district-feed`, {
//           method: 'GET',
//           headers: {
...
//         console.log('API Response in community feed:', response);

//         if (!response.ok) {
//           throw new Error('Failed to fetch district reports');
//         }
//         const data = await response.json();
//         const formattedReports = data.reports.map(report => ({
//           ...report,
//           isUpvoted: report.upvotes.includes(user._id)
//         }));
...
//     switch (sortBy) {
//       // NOTE: "nearMe" sorting requires a real user location and report location data
//       // and would require a more complex distance calculation function.
//       case 'nearMe':
...
//               ? {
//                   ...report,
//                   upvotes: updatedReport.report.upvotes,
//                   isUpvoted: updatedReport.report.upvotes.includes(user._id)
//                 }
//               : report
//           )
//         );
...
//         <h2 className="text-xl font-semibold text-gray-800">Please log in to view the community feed.</h2>
//         <p className="text-gray-600 mt-2">
//           Your reports will appear here once you are authenticated.
//         </p>
//       </div>
//     );
//   }
...
//             <div key={report._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow lg:p-8">
...
//                     </svg>
//                     {report.isUpvoted ? 'Upvoted' : 'Upvote'} ({report.upvotes.length})
//                   </button>
//                 </div>
//               </div>
//             </div>
